...
mod tests {
    ...
    fn test_split_slice_by_nth() {
        let slice = &[1,2,3];
        assert_eq!(split_slice_by_nth(slice, 0).unwrap().to_owned(), (&slice[..0], &slice[0..]));
        assert_eq!(split_slice_by_nth(slice, 1).unwrap().to_owned(), (&slice[..1], &slice[1..]));
        assert_eq!(split_slice_by_nth(slice, 2).unwrap().to_owned(), (&slice[..2], &slice[2..]));
